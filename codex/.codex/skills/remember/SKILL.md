...
| `/remember --global <fact>` | Force global destination (`~/.codex/KNOWLEDGE.md`). Still runs bar. |
...
**1.** Read `~/.codex/references/MEMORY-STANDARD.md` first — authoritative source for entry format, routing rules, deduplication, and the promotion bar.

**2.** Extract the fact from the argument. Strip `--global` and `--force` flags before processing.

**3.** Determine destination:
   - `--global` present → `~/.codex/KNOWLEDGE.md`
   - Otherwise → `KNOWLEDGE.md` in the current project root (walk up from cwd for `.git` marker); fall back to `~/.codex/KNOWLEDGE.md` if not in a project
...
**4.** Unless `--force`:
   ...
   - If any test fails: name the failing test(s), suggest the correct destination (TODOS.md for open work, AGENTS.md for rules), stop — do not write
