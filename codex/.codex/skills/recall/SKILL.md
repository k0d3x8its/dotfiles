...
|---|---|---|---|
| L1 — index | `.memory/EPISODIC-INDEX.md` + `KNOWLEDGE.md` tiers | ~50–100 tok | Always. Compact hit list. |
...
- Current project: `.memory/SESSION-LOG.md`, `.memory/EPISODIC-INDEX.md`
- Local `KNOWLEDGE.md` (project root)
- Global `~/.codex/KNOWLEDGE.md`

Scratchpad (`~/.codex/projects/<hash>/memory/`) is **out of default scope** — it is per-machine, unvetted, and noisy.
...
- `.memory/EPISODIC-ARCHIVE.md` per project (label hits `[archived]`)
- Scratchpad: `~/.codex/projects/<hash>/memory/` (label hits `[scratch]`)
...
**3.** Layer 1 — grep and present compact hit list:
   ...
   1. `<project-root>/.memory/EPISODIC-INDEX.md` — grep for query string; emit matching lines (already one-line format: ISO timestamp + fields)
   2. `<project-root>/KNOWLEDGE.md` — grep for query string; emit matching bullet(s)
   3. `~/.codex/KNOWLEDGE.md` — grep for query string; emit matching bullet(s)
   4. `<project-root>/.memory/SESSION-LOG.md` — grep for query string in section headers; emit matching `## Session` header lines only (not full blocks)

   For **`--deep` scope**: walk every direct subdirectory of `~/dev/` that looks like a project (contains `KNOWLEDGE.md`, `.memory/SESSION-LOG.md`, or `.memory/EPISODIC-INDEX.md`). For each:
   - Apply git-crypt guard before grepping `KNOWLEDGE.md`
   - Grep `.memory/EPISODIC-INDEX.md` (if present), `KNOWLEDGE.md` (if unlocked), `.memory/EPISODIC-ARCHIVE.md` (label `[archived]`)
   - Also grep scratchpad: enumerate `~/.codex/projects/*/memory/*.md` — label any hit `[scratch]`
...
**5.** Layer 2 — expand on request:
   - For an `.memory/EPISODIC-INDEX.md` hit: find the ISO timestamp in the index, then search `.memory/SESSION-LOG.md` for the `## Session` block nearest that date. Read and print that block only (from its `## Session` header to the next `---` separator).
   - For a `KNOWLEDGE.md` hit: print the surrounding 5 bullets (2 before, the match, 2 after) for context.
   - For a `.memory/SESSION-LOG.md` header hit: read and print that full `## Session` block.
   - For an `[archived]` hit: read `.memory/EPISODIC-ARCHIVE.md` and print the matching line + its 2 surrounding lines.
...
**6.** Layer 3 — full read on `full <n>` request:
   - For an episodic/session hit: read the complete `.memory/SESSION-LOG.md` block (or the full `.memory/EPISODIC-ARCHIVE.md` entry).
...
| `[archived]` | Hit came from `.memory/EPISODIC-ARCHIVE.md` (older than live index) |
