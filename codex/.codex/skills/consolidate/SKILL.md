...
## Codex Instructions — Manual Mode (Read Before Executing)

**1.** Execute immediately. No clarifying questions.

**2.** Read `~/.codex/references/MEMORY-STANDARD.md` — authoritative source for the promotion bar, entry format, routing, and dedup rules. This step is mandatory before running any bar test.

**3.** Determine sweep scope:
   - Read `~/dev/.consolidation-marker` (JSON). Extract the timestamp for the current project (key = `basename` of cwd's project root; `[machine]` for `~/dev`).
...
**6.** Run the promotion gate — identical to /checkpoint Step 7:
   ...
   - **NOT A RULE** — empirical observation, not a normative instruction (rules → AGENTS.md).
...
**9.** For candidates that fail the bar:
   ...
   - NOT A RULE failure → suggest adding to `AGENTS.md` (do not write automatically)
...
## Codex Instructions — Scheduled Mode
