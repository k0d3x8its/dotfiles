...
**Mapping:** see **Trello Sync Rules** in `~/.codex/AGENTS.md` (canonical) for the Goal→card / Micro-Goal→checklist / Task→item rule and the `[trello:ID]` skip rule. Skill-specific detail: both `- [ ]` and `- [x]` lines become checklist items (completed tasks still sync).
...
**If `.codex/trello-board` exists** in the project root: read the board name from that file and use it.

**Otherwise**: run `trello board:list --format json`, display the board names, and ask:
> "Which Trello board should I sync to? I can save your choice to `.codex/trello-board` so you don't have to pick again."

If the user agrees to save: write the board name (just the name, no quotes) to `.codex/trello-board`.
...
- List names must exist on the target board — standard KOS Kanban columns are defined in `~/.codex/AGENTS.md`. Any board layout works as long as a `Back Log` list exists (or the user is asked which list to use)
...
- `.codex/trello-board` is project-local — different projects can point to different boards
