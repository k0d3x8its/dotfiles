...
repo with a remote (AGENTS.md File Taxonomy): `KNOWLEDGE.md`, `TODOS.md`,
...
### 6. Harness surface (only when auditing a repo that ships Codex config)

For repos containing `.codex/`, hooks, or skills (e.g. dotfiles): check hooks
...
```bash
rg -n '[\x{200B}-\x{200F}\x{202A}-\x{202E}\x{2066}-\x{2069}]' claude/.codex/skills/ claude/.codex/hooks/
```
