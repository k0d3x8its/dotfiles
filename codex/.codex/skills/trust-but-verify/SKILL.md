...
points below (encoded as a session rule in `~/.codex/AGENTS.md`).
...
1. **Resolve the verify command** via `~/.codex/skills/trust-but-verify/detect.md`.
...
- No verify command resolvable at all → say so explicitly, recommend adding one to the
  project's AGENTS.md/KNOWLEDGE.md (detect.md priority 1), and treat all claims as
  unproven until then.
