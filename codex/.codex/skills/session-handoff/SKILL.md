...
argument-hint: "What will the tangent session focus on? (optional — Codex will infer from conversation if omitted)"
---
...
**2.** Light TODOS touch only:
   ...
   - If forking reveals a new open item, append it to `TODOS.md` with tags from `~/.codex/AGENTS.md` (`- [ ]` + tags; untagged = Medium). Update the `Last updated:` date.
...
**4.** Build the **Tangent Re-Entry Prompt**:
   ...
   At session start: read TODOS.md and KNOWLEDGE.md (local + global) — if either is git-crypt ciphertext, unlock first per `~/.codex/references/git-crypt-lock-check.md`. For what's next across projects, read `~/dev/.memory/TRIAGE-BLOCK.md`.
