...
Files: path/to/file, path/to/other | Tags: [FEAT] [BUG] {annotation tags from AGENTS.md describing this session's work}
...
> Read `.memory/SESSION-LOG.md`, `TODOS.md`, and `KNOWLEDGE.md` (local + global) — if any reads as git-crypt ciphertext, unlock first per `~/.codex/references/git-crypt-lock-check.md` (the manual fetch→unlock pipe, run from inside the repo). For what's next across projects, read `.memory/TRIAGE-BLOCK.md`.
> First action: {step}}"
...
**2.** Read:
   ...
   - `~/.codex/KNOWLEDGE.md` (global knowledge) — same reason
...
**3.** Update `TODOS.md` in-place:
   ...
   - **New open items** → append with tags from the TODO Tags system in `~/.codex/AGENTS.md`. Every item begins `- [ ]` then tags. Untagged = Medium.
...
**5b.** Emit the machine-greppable `Files:` / `Tags:` line under **Files Touched** — a single unwrapped line listing the files this session touched and the annotation tags (`[FEAT]`/`[BUG]`/… from `AGENTS.md`) that classify the work. This is the curated counterpart to the auto-captured `.memory/EPISODIC-INDEX.md` line; `/recall` Layer 1 filters on it.
...
**7.** KNOWLEDGE.md promotion gate — scan `### Decisions Made` and `### Gotchas / Notes` from the block just written for facts eligible for KNOWLEDGE.md.
   ...
   First, read `~/.codex/references/MEMORY-STANDARD.md` — authoritative source for the promotion bar, entry format, routing, and dedup rules.
...
**9.** Changelog: use `/changelog` manually if this session produced changelog-worthy changes. Do not auto-update inline — AGENTS.md delegates this to `/changelog`.
...
Works around Codex's **1-hour cache TTL**. Cached tokens cost ~10% of normal input; let them expire and you pay full price again.
...
- Project-level `.codex/AGENTS.md` exists: note conventions relevant to open items; don't reproduce it.
