...
- Security: when code accepts a value affecting money/access/identity, reads/writes a store or policy file, or selects a file/URL/destination from a value, read `~/.claude/references/security/SECURITY-STANDARD.md` (Universal MUSTs + router) plus at most the ONE or TWO triggered sector files — `CLIENT-TRUST` `DATA-STORE` `RESOURCE-ACCESS` (pilot; 5 more sectors designed but not yet built, see the router). Never load the whole `security/` dir. If 3+ sectors trigger, load them all but say so explicitly in the response (overflow-flag protocol, see router) — don't silently absorb the budget violation.
...
| Write-time security guidance, routed by domain (pilot: `CLIENT-TRUST`, `DATA-STORE`, `RESOURCE-ACCESS`; 5 more designed, not built) | `~/.claude/references/security/` — `SECURITY-STANDARD.md` (router) + at most two triggered sector files |
