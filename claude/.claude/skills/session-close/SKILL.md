...
**2.** Light TODOS touch only:
   - New open items discovered this session → append to `TODOS.md` with tags from `~/.claude/CLAUDE.md`. Update `Last updated:` date.
...
**4.** Build the **Resume Re-Entry Prompt**:
   ...
   At session start: read TODOS.md. For what's next, read TRIAGE-BLOCK.md.
   ──────────────────────────────────────────────
   ```
