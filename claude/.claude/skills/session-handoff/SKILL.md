...
**Trigger:** `/handoff`
**Purpose:** Fork a _tangent_ off the current session. Print a focused re-entry prompt so a fresh session can chase a side-issue with clean context — then `/handoff-return` merges its findings back into the still-alive main session.
...
**NEW-FORMAT** (`.work/plan/` exists): if the tangent corresponds to an existing
`TODOS.md` index line, note it. If forking reveals a new open item, append an
index line (`- [ ]` + tags + title); spill to `.work/todos/<slug>.md`,
pointer added, only past ~150 words — shorter items stay inline with no pointer.
...
At session start: read TODOS.md and KNOWLEDGE.md (local + global) — if either is git-crypt ciphertext, unlock first per `~/.claude/references/git-crypt-lock-check.md`. For what's next across projects, read `~/dev/.memory/TRIAGE-BLOCK.md`.
