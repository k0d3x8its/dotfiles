...
| Episodic | Time-indexed events | `SESSION-LOG.md` + `ARCHIVE-LOG.md` + `EPISODIC-INDEX.md` | ⚠️ building |
...
- `EPISODIC-INDEX.md` = **complete but thin** — one auto-captured line per session (every session, metadata only). The searchable spine.
- `SESSION-LOG.md` = **sparse but rich** — full narrative, only for `/checkpoint`-ed sessions. The curated "why."
...
|---|---|---|
| `SESSION-LOG.md` | Curated session narratives (checkpoint only) | No — git-crypt encrypted + committed |
| `ARCHIVE-LOG.md` | Rotated-out old SESSION-LOG blocks | No — committed |
| `EPISODIC-INDEX.md` | One-line-per-session auto-captured spine | Yes — runtime artifact |
...
session ends
    ↓
SessionEnd hook → update-episodic → EPISODIC-INDEX.md   (zero tokens, every session)
    ↓
/checkpoint → SESSION-LOG.md block with Files:/Tags: line  (human-curated, rich)
    ↓
