...
| **Comments**        | Comment explains WHAT the code does rather than WHY — sign the code itself is unclear. Also covers stale comments: a WHY that no longer matches the code below it is worse than no comment |
...
| **Boolean Blindness**       | Call site reads `foo(true, false, true)` — flags are opaque without checking the signature |
...
| **CQS Violation**       | Function both returns a value and mutates state — unsafe to reorder, retry, or call twice |
...
| **Circular Dependency**                           | Two modules import/require each other — no clear ownership direction             |
...
| **Complex / Nested Conditional** | Deep `if`/`else` nesting or a sprawling boolean expression; intent is buried in structure |
...
_(Brown et al — AntiPatterns, 1998)_
...
_(Meszaros — xUnit Test Patterns; general practice)_
...
| **Conditional Test Logic** | Branching (`if`, loops) inside a test body — the test itself is ambiguous about what it verifies | Split into separate tests, or parametrize (test-structure work — `/tdd`, not `/code-refactor`) |
