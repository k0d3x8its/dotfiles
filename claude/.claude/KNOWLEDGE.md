...
- ccusage token accounting: `cacheReadInputTokens` is ~90%+ of `totalTokens` (live: ~9M of ~10M in one active block). For any usage-proportion metric, count `inputTokens + outputTokens + cacheCreation` and EXCLUDE cache-read — dividing `totalTokens` by a ceiling inflates the result ~10–20×.
- ccusage `--json` schema quirk: the `blocks` command keys cache-creation as `cacheCreationInputTokens`; the `daily` command keys the same quantity as `cacheCreationTokens`. Different keys, same number.
