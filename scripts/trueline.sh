...
_trueline_vimode_segment() {
	if [[ "$TRUELINE_SHOW_VIMODE" = true ]]; then
		...
		# Cosmetic-only "submitted" marker: repaints the already-printed
		# prompt's mode badge from insert-color to cmd-color in place once
		# Enter is pressed, via cursor movement + raw ANSI codes in PS0.
		# Deliberately does NOT touch readline keymaps or the real vi
		# insert/command mode -- an earlier attempt at this used a RETURN
		# keybind to force command mode, which desynced insert-mode-by-
		# default on the next prompt and broke typing entirely. This runs
		# after the line is already submitted, so it can't affect editing.
		PS0='$(_trueline_freeze_vimode_badge)'
	else
		bind "set show-mode-in-prompt off"
	fi
}
_trueline_freeze_vimode_badge() {
	[[ "$TRUELINE_SHOW_VIMODE" = true ]] || return
	local ins_symbol="${TRUELINE_SYMBOLS[vimode_ins]}"
	local badge_width=$((${#ins_symbol} + 2))
	local label
	printf -v label "%-${badge_width}.${badge_width}s" "${TRUELINE_SYMBOLS[vimode_cmd]}"
	local fg="${TRUELINE_COLORS[${TRUELINE_VIMODE_CMD_COLORS_STYLE[0]}]}"
	local bg="${TRUELINE_COLORS[${TRUELINE_VIMODE_CMD_COLORS_STYLE[1]}]}"
	printf '\033[s\033[1A\033[1G\033[1m\033[38;2;%sm\033[48;2;%sm%s\033[0m\033[u' \
		"$fg" "$bg" "$label"
}
