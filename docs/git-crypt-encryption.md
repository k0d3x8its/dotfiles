...
- **Global gitignore** (`git/.gitignore_global` → `~/.gitignore_global` via
  `core.excludesfile`) for planning files — `TODOS.md`, `.work/FINDINGS.md`, `.work/PROGRESS.md`,
  `.work/PLAN.md`, `.memory/SESSION-LOG.md`, `RELEASE-NOTES.md`, `.memory/ARCHIVE-LOG.md`. These are
...
2. **Planning files are lost across machines.** Because they're globally ignored, work
   notes (`TODOS.md`, `.work/FINDINGS.md`, `.work/PROGRESS.md`, …) never sync. The user wants them
...
| `.work/FINDINGS.md` | **Encrypt + commit** | Investigation notes; can hold sensitive detail. |
| `.work/PROGRESS.md` | **Encrypt + commit** | Progress log; worth syncing. |
| `.work/PLAN.md` | **Encrypt + commit** | Structured plan; may hold sensitive detail. |
| `.memory/SESSION-LOG.md` | **Encrypt + commit** | Session narrative + decisions; the durable record. |
...
| `.memory/ARCHIVE-LOG.md` | **Keep local** (opt-in encrypt) | Rotated archive of `.memory/SESSION-LOG.md`; large, low value to sync. |
| `.memory/TRIAGE-BLOCK.md` | **Keep local** (opt-in encrypt) | Derived from `TODOS.md` by the triage script; regenerated, so committing it is redundant. |
...
.work/FINDINGS.md     filter=git-crypt diff=git-crypt
.work/PROGRESS.md     filter=git-crypt diff=git-crypt
.work/PLAN.md    filter=git-crypt diff=git-crypt
...
> Ask: *"Encrypt this project's planning files (TODOS.md, .work/FINDINGS.md, .work/PROGRESS.md,
> .work/PLAN.md, .memory/SESSION-LOG.md, KNOWLEDGE.md) with git-crypt so they commit safely? (yes / skip)"*
> If **yes**:
