...
SessionEnd hook → episodic_index.py → update-episodic
    ↓
per-project  EPISODIC-INDEX.md   (one line per session, thin)
global       ~/dev/EPISODIC-INDEX.md  (roll-up across all projects)
    ↓
...
| `EPISODIC-INDEX.md` | One-line-per-session spine (per project) | Yes — runtime artifact |
| `~/dev/EPISODIC-INDEX.md` | Global roll-up across all projects | Yes — runtime artifact |
| `EPISODIC-ARCHIVE.md` | Rotated-out old index lines | Yes — runtime artifact |
...
Derives the project from `cwd`, computes files-touched via `git diff <baseline>..HEAD` + live working tree, and appends one structured line to both the per-project and global `EPISODIC-INDEX.md`.
...
**Decay / rotation:** once `EPISODIC-INDEX.md` exceeds `MAX_LINES`, the oldest half moves to `EPISODIC-ARCHIVE.md`. The global index stays thin; the archive preserves history.
...
| Layer | Source | Cost |
|-------|--------|------|
| L1 | `EPISODIC-INDEX.md` + `KNOWLEDGE.md` grep (current project) | Cheap |
| L2 | Expand matching SESSION-LOG.md blocks | Medium |
| L3 (`--deep`) | Fan out across all `~/dev` projects + `EPISODIC-ARCHIVE.md` | More |
...
Sweeps `EPISODIC-INDEX.md` and `SESSION-LOG.md` entries since the last consolidation marker, runs each candidate through the 4-test bar (SETTLED · NON-OBVIOUS · NOT A RULE · DURABLE), and routes approved facts through `/remember`'s dedup logic into `KNOWLEDGE.md`.
